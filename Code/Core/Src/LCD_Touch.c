...
void TP_Temp(void)
{
	char *New_Temp_string = (char*)malloc(1);
    ...
    if (sTP_DEV.chStatus & TP_PRESS_DOWN) {		//Press the button
        ...
        if (sTP_Draw.Xpoint < sLCD_DIS.LCD_Dis_Column &&
            ...
            if(sLCD_DIS.LCD_Dis_Column > sLCD_DIS.LCD_Dis_Page) 
						{
							if ((sTP_Draw.Xpoint > 40 && sTP_Draw.Xpoint < 205) && (sTP_Draw.Ypoint > 120 && sTP_Draw.Ypoint < 190) && STATE == 1)
							{
								TP_Show_Temps();
								STATE = 2;
							}
							else if (sTP_Draw.Xpoint < 100 && sTP_Draw.Ypoint > 280 && STATE == 2)
							{
							...
							else if ((sTP_Draw.Xpoint > 100 && sTP_Draw.Xpoint < 170) && (sTP_Draw.Ypoint > 220 && sTP_Draw.Ypoint < 260) && STATE == 2)
							{
								New_Temp -= 0.5;
								int ipart = (int)New_Temp; 
								float fpart = New_Temp - (float)ipart; 

								int i = intToStr(ipart, New_Temp_string, 0); 

								if (1 != 0) 
								{ 
									New_Temp_string[i] = '.';
									fpart = fpart * pow(10, 1); 
									intToStr((int)fpart, New_Temp_string + i + 1, 1); 
								}
								LCD_SetArealColor(290, 140, 360, 160, FONT_BACKGROUND);
								GUI_DisString_EN(290, 140, New_Temp_string, &Font24, FONT_BACKGROUND, RED);
								HAL_Delay(200);
							}
							else if ((sTP_Draw.Xpoint > 310 && sTP_Draw.Xpoint < 380) && (sTP_Draw.Ypoint > 205 && sTP_Draw.Ypoint < 275) && STATE == 2)
							{
								New_Temp += 0.5;
								int ipart = (int)New_Temp; 
								float fpart = New_Temp - (float)ipart; 
...
void TP_Show_Main(void)
{
    ...
    if(sLCD_DIS.LCD_Dis_Column > sLCD_DIS.LCD_Dis_Page) 
		{
			GUI_DisString_EN(16, 15, "Urzadzenie zarzadzajace temperatura oraz  oswietleniem w pokoju", &Font16, WHITE, BLUE);
			...
			GUI_DrawRectangle(40, 120, 205, 190, RED, DRAW_EMPTY, DOT_PIXEL_2X2);
			GUI_DrawRectangle(275, 120, 440, 190, RED, DRAW_EMPTY, DOT_PIXEL_2X2);
			GUI_DisString_EN(45, 147, "TEMPERATURA", &Font20, WHITE, BLUE);
			GUI_DisString_EN(280, 147, "OSWIETLENIE", &Font20, WHITE, BLUE);
			GUI_DisString_EN(190, 270, "Wykonawcy:", &Font16, WHITE, BLUE);
			GUI_DisString_EN(135, 285, "inz. Krzysztof Pokora", &Font16, WHITE, BLUE);
			GUI_DisString_EN(120, 300, "mgr inz. Arkadiusz Pokora", &Font16, WHITE, BLUE);
    } 
		else 
...
void TP_Show_Temps(void)
{	
		char *New_Temp_string = (char*)malloc(13);
    ...
    if(sLCD_DIS.LCD_Dis_Column > sLCD_DIS.LCD_Dis_Page) 
		{
			...
			GUI_DisString_EN(16, 90, "Aktualna temperatura", &Font16, WHITE, BLUE);
			GUI_DisString_EN(256, 90, "Zadana temperatura", &Font16, WHITE, BLUE);		

			GUI_DrawCircle(165, 146, 4, RED, DRAW_EMPTY, DOT_PIXEL_1X1);
			GUI_DisString_EN(170, 140, "C", &Font24, WHITE, RED);
			
			LCD_SetArealColor(290, 140, 360, 160, FONT_BACKGROUND);
			GUI_DisString_EN(290, 140, New_Temp_string, &Font24, FONT_BACKGROUND, RED);
			GUI_DrawCircle(375, 146, 4, RED, DRAW_EMPTY, DOT_PIXEL_1X1);
			GUI_DisString_EN(380, 140, "C", &Font24, WHITE, RED);
			
			GUI_DrawLine(100, 240, 170, 240, BLUE, LINE_SOLID, DOT_PIXEL_3X3);
			GUI_DrawLine(310, 240, 380, 240, RED, LINE_SOLID, DOT_PIXEL_3X3);
			GUI_DrawLine(345, 205, 345, 275, RED, LINE_SOLID, DOT_PIXEL_3X3);
			
			GUI_DisString_EN(16, 295, "Powrot", &Font20, WHITE, BLUE);
...
void TP_Show_Temp(float temperature)
{
	if (STATE == 2)
	{
		char *Temp_string = (char*)malloc(12);
		...
		int ipart = (int)temperature; 
		float fpart = temperature - (float)ipart; 

		int i = intToStr(ipart, Temp_string, 0); 

		if (1 != 0) 
		{ 
			Temp_string[i] = '.';
			fpart = fpart * pow(10, 1); 
			intToStr((int)fpart, Temp_string + i + 1, 1); 
		}

		LCD_SetArealColor(80, 140, 150, 160, FONT_BACKGROUND);
		GUI_DisString_EN(80, 140, Temp_string, &Font24, FONT_BACKGROUND, RED);
		
		if (temperature < New_Temp)
		{
			LCD_SetArealColor(40, 125, 70, 175, FONT_BACKGROUND);
			GUI_DrawLine(55, 130, 55, 170, RED, LINE_SOLID, DOT_PIXEL_2X2);
			GUI_DrawLine(55, 130, 45, 150, RED, LINE_SOLID, DOT_PIXEL_2X2);
			GUI_DrawLine(55, 130, 65, 150, RED, LINE_SOLID, DOT_PIXEL_2X2);
			
			HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);
		}
		else if(temperature > New_Temp)
		{
			LCD_SetArealColor(40, 125, 70, 175, FONT_BACKGROUND);
			GUI_DrawLine(55, 130, 55, 170, BLUE, LINE_SOLID, DOT_PIXEL_2X2);
			GUI_DrawLine(55, 170, 45, 150, BLUE, LINE_SOLID, DOT_PIXEL_2X2);
			GUI_DrawLine(55, 170, 65, 150, BLUE, LINE_SOLID, DOT_PIXEL_2X2);
